...
function SubscriptionPage() {
  ...
  return (
    <>
      <PageHeader
        title="Subscription"
        description="Your active plan, resource usage, and billing management."
      />
      ...
      <Card className="mb-6 overflow-hidden border-border/60 shadow-xs">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between relative z-10">
            <div className="min-w-0">
              ...
              <div className="text-xs sm:text-sm opacity-90 mt-1.5">
                {subscription.billing} billing · Renews on{" "}
                <strong className="font-semibold text-white">{subscription.expiryDate}</strong>
              </div>
            </div>
            <div className="flex flex-wrap gap-2.5 shrink-0">
              <Button
                ...
                onClick={() => toast.success("Renewal process started")}
              >
                Renew Plan
              </Button>
              <Button
                size="sm"
                className="h-10 px-4 bg-accent-red hover:bg-accent-red/90 text-accent-red-foreground font-semibold shadow-xs"
              >
                Upgrade Plan
              </Button>
            </div>
        ...
        <CardContent className="p-5 sm:p-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-4 bg-card">
          ...
          <UsageBar
            label="Instructors"
            used={subscription.instructorsUsed}
            limit={subscription.instructorLimit}
            unit=""
          />
          <UsageBar
            label="Storage Space"
      ...
      <div className="mb-4">
        <PageHeader
          ...
          description="Choose the plan that fits your driving school growth."
        />
      </div>
          ...
          return (
            <Card
              ...
              className={cn(
                ...
                p.popular
                  ? "border-2 border-brand shadow-sm ring-2 ring-brand/15"
                  : "border-border",
              )}
            >
              {p.popular && (
                <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 z-10">
                  <StatusBadge
                    tone="brand"
                    className="shadow-xs font-semibold px-3 py-1 text-[11px] uppercase tracking-wider"
                  >
                    Most Popular
                  </StatusBadge>
                </div>
          ...
          <div className="flex items-center gap-3.5">
            ...
            <div>
              ...
              <div className="text-xs sm:text-sm text-muted-foreground mt-0.5">
                Renew or upgrade instantly with eSewa digital wallet. Zero waiting time and
                automated instant receipt.
              </div>
            </div>
          </div>
          <Button
