  ...
  head: () => ({
    meta: [
      { title: "Students — DriveSiksha" },
      {
        name: "description",
        content:
          "Search, filter and manage all driving school students, their instructor, lesson progress and payment status.",
      },
    ],
  }),
});
        ...
        <div className="flex justify-between items-start border-b pb-4 mb-4">
          <div>
            ...
            <div className="text-xs text-gray-600 mt-1">
              {currentSchool.name} · {currentSchool.address}
            </div>
            <div className="text-xs text-gray-600">
              Contact: {currentSchool.phone} · {currentSchool.email}
            </div>
          </div>
          ...
          <div className="flex flex-col gap-2 lg:flex-row lg:items-center">
            <div className="relative flex-1 min-w-0">
              ...
              <Input
                ...
                placeholder="Search by name, phone, student ID, or receipt no…"
                className="pl-9 h-10"
              />
            </div>
        ...
        <div>
            ...
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0 flex-1">
                <Link
                  to="/students/$id"
                  params={{ id: s.id }}
                  className="font-semibold text-base truncate block hover:underline group-hover:text-brand transition-colors"
                >
                  {s.name}
                </Link>
                <div className="flex items-center gap-2 text-xs text-muted-foreground mt-0.5 flex-wrap">
                  <span className="font-mono bg-muted/70 px-1.5 py-0.5 rounded text-[11px] font-medium">
                    {s.studentId}
                  </span>
          ...
          <div className="mt-3.5 space-y-1.5 text-xs text-muted-foreground bg-muted/20 p-2.5 rounded-lg border border-border/50">
            ...
            <div className="flex items-center gap-2 truncate">
              ...
              <span>
                Instructor: <strong className="font-medium text-foreground">{s.instructor}</strong>
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3 pt-1 border-t border-border/40">
          <div>
            <div className="flex justify-between items-center text-xs mb-1.5">
              ...
              <span className="font-semibold text-foreground">
                {s.completedLessons}/{s.totalLessons} ({progressPct}%)
              </span>
            </div>
          ...
          <div className="flex items-center justify-between gap-2 pt-1 flex-wrap">
            ...
            <div className="text-xs font-semibold text-foreground ml-auto">
              {formatNPR(s.amountPaid)}{" "}
              <span className="text-muted-foreground font-normal">/ {formatNPR(s.courseFee)}</span>
            </div>
          </div>
        </div>
      </CardContent>
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end">
        <DropdownMenuItem asChild>
          <Link to="/students/$id" params={{ id: studentId }}>
        ...
        <DropdownMenuSeparator />
        <DropdownMenuItem onClick={() => toast.success("Student archived")}>
          <Archive className="h-4 w-4 mr-2" />
          Archive
        </DropdownMenuItem>
        <DropdownMenuItem
          className="text-destructive"
          onClick={() => toast.error("Cannot delete active student")}
        >
