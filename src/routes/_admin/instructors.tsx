...
export const Route = createFileRoute("/_admin/instructors")({
  ...
  head: () => ({
    meta: [
      { title: "Instructors — DriveSiksha" },
      { name: "description", content: "Manage all driving instructors, schedules and licences." },
    ],
  }),
});
...
function InstructorsPage() {
  ...
  return (
    <>
      <PageHeader
        title="Instructors"
        description={filtered.length + " instructors"}
        actions={
          <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
            <DialogTrigger asChild>
              <Button
                size="sm"
                className="h-9 bg-accent-red hover:bg-accent-red/90 text-accent-red-foreground"
              >
                ...
                <TableRow key={i.id}>
                  <TableCell>
                    <div className="flex items-center gap-3">
                      <div className="h-9 w-9 rounded-full bg-sky/15 text-sky grid place-items-center font-semibold text-xs">
                        {i.name
                          .split(" ")
                          .map((n: string) => n[0])
                          .join("")
                  ...
                  <TableCell>
                    <StatusBadge tone={i.leaveStatus === "available" ? "success" : "warning"}>
                      {i.leaveStatus.replace("_", " ")}
                    </StatusBadge>
                  </TableCell>
                  <TableCell>
                    <StatusBadge tone={i.accountStatus === "active" ? "success" : "neutral"}>
                      {i.accountStatus}
                    </StatusBadge>
                  </TableCell>
          ...
          <Card key={i.id}>
            <CardContent className="p-4">
              <div className="flex items-start gap-3">
                <div className="h-11 w-11 rounded-full bg-sky/15 text-sky grid place-items-center font-semibold shrink-0">
                  {i.name
                    .split(" ")
                    .map((n: string) => n[0])
                    .join("")
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-2 text-xs text-muted-foreground">
                    {i.assignedStudents} students · {i.todayLessons} lessons today
                  </div>
                </div>
              </div>
            </CardContent>
