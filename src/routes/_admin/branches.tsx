...
export const Route = createFileRoute("/_admin/branches")({
  ...
  head: () => ({
    meta: [
      { title: "Branches — DriveSiksha" },
      {
        name: "description",
        content: "Manage all your driving school branches, staff and revenue.",
      },
    ],
  }),
});
  ...
  return (
    <>
      <PageHeader
        ...
        description={`${subscription.branchesUsed} of ${subscription.branchLimit} branches used on your ${subscription.plan} plan`}
        actions={
          <Button
            size="sm"
            disabled={atLimit}
            className="h-9 bg-accent-red hover:bg-accent-red/90 text-accent-red-foreground disabled:opacity-50"
            onClick={() =>
              atLimit
                ? toast.error("Plan limit reached. Upgrade to add more branches.")
                : toast.success("Add branch modal opened")
            }
          >
      ...
      <Card className="mb-4">
        <CardContent className="p-4 sm:p-5">
          <div className="flex items-center justify-between mb-2">
            <div>
              ...
              <div className="text-xs text-muted-foreground">
                {subscription.branchesUsed} of {subscription.branchLimit} branches on{" "}
          ...
          <Progress
            value={(subscription.branchesUsed / subscription.branchLimit) * 100}
            className="h-2"
          />
        </CardContent>
      </Card>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {branches.map((b) => (
          <Card
            key={b.id}
            className="group relative overflow-hidden transition-all duration-200 hover:shadow-md hover:border-brand/40 flex flex-col justify-between"
          >
            ...
            <CardContent className="p-4 sm:p-5 flex flex-col justify-between h-full gap-4">
              <div>
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-start gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <div className="font-semibold text-base truncate group-hover:text-brand transition-colors">
                        {b.name}
                      </div>
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem onClick={() => toast.success(`Editing ${b.name}`)}>
                        <Edit className="h-4 w-4 mr-2" />
                        Edit branch
                      </DropdownMenuItem>
                      <DropdownMenuItem onClick={() => toast.error(`Deactivated ${b.name}`)}>
                ...
                <div className="mt-3.5 space-y-1.5 text-xs text-muted-foreground bg-muted/20 p-2.5 rounded-lg border border-border/50">
                  ...
                  <div className="flex items-center gap-2 truncate">
                    ...
                    <span>
                      Manager: <strong className="font-medium text-foreground">{b.manager}</strong>
                    </span>
                  </div>
                </div>
                ...
                <div className="mt-3.5 grid grid-cols-3 gap-1.5 p-3 rounded-xl bg-muted/30 border border-border/50">
                  ...
                  <div className="text-center min-w-0">
                    ...
                    <div
                      className="text-[11px] sm:text-xs font-bold text-foreground mt-0.5 whitespace-nowrap overflow-hidden text-ellipsis"
                      title={formatNPR(b.monthlyRevenue)}
                    >
              ...
              <div className="pt-2 flex items-center justify-between border-t border-border/40">
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  className="h-8 text-xs hover:text-brand"
                  onClick={() => toast.info(`Viewing details for ${b.name}`)}
                >
                  View details
                </Button>
              </div>
